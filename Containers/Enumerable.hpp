...
public:
	...
	{
		Iterator Beg( Enumerable< ValueType >::Begin() );
		Iterator End( Enumerable< ValueType >::End() );
	...
	inline auto Combine( Enumerable< ValueType >& a_Enumerable )
	{
		...
		for ( Iterator Beg( Begin() ); LengthA > 0; --LengthA, ++Beg )
		{
			Result.emplace_back( *Beg );
		}

		for ( Iterator Beg( a_Enumerable.Begin() ); LengthB > 0; --LengthB, ++Beg )
		{
			Result.emplace_back( *Beg );
		}
	...
	inline auto Difference( Enumerable< ValueType >& a_Enumerable )
	{
	...
	inline auto Intersection( Enumerable< ValueType >& a_Enumerable )
	{
		set< ValueType*, LessPtr< ValueType > > SetA;
		set< ValueType*, LessPtr< ValueType > > SetB;
		vector< Reference< ValueType > > Result;
	...
	{
		return Enumerable< ValueType >( Begin() + a_Offset, Begin() + a_Offset + a_Length );
	}
	...
	inline void Swap( Enumerable< ValueType >& a_Enumerable )
	{
		swap( m_Begin, a_Enumerable.m_Begin );
		swap( m_End, a_Enumerable.m_End );
		swap( m_Size, a_Enumerable.m_Size );
		swap( m_Operator, a_Enumerable.m_Operator );
	}

	inline auto Symmertry( Enumerable< ValueType >& a_Enumerable )
	{
		set< ValueType*, LessPtr< ValueType > > SetA;
		set< ValueType*, LessPtr< ValueType > > SetB;
		vector< Reference< ValueType > > Result;
	...
	inline auto Symmertry( const Enumerable< ValueType >& a_Enumerable ) const
	{
		set< const ValueType*, LessPtr< ValueType > > SetA;
		set< const ValueType*, LessPtr< ValueType > > SetB;
		vector< CReference< ValueType > > Result;
	...
	inline bool TrueForAll( const Predicate< ValueType& >& a_Predicate )
	{
		...
		for ( Iterator Beg( Begin() ); Length > 0; --Length, ++Beg )
		{
			if ( !a_Predicate.Invoke( *Beg ) )
			{
				return false;
	...
	inline bool TrueForAll( const Predicate< const ValueType& >& a_Predicate ) const
	{
		...
		for ( CIterator Beg( Begin() ); Length > 0; --Length, ++Beg )
		{
			if ( !a_Predicate.Invoke( *Beg ) )
			{
				return false;
	...
	inline bool TrueForAll( Iterator a_Begin, const Iterator& a_End, const Predicate< ValueType& >& a_Predicate )
	{
		for ( ; a_Begin != a_End; ++a_Begin )
		{
			if ( !a_Predicate.Invoke( *a_Begin ) )
	...
	inline auto Union( Enumerable< ValueType >& a_Enumerable )
	{
	...
	inline auto Union( const Enumerable< ValueType >& a_Enumerable ) const
	{
	...
	inline void ZeroAll()
	{
		...
		for ( Iterator Beg( Begin() ); Length > 0; --Length, ++Beg )
		{
			memset( &*Beg, 0, sizeof( ValueType ) );
		}
	}

	inline auto Zero( size_t a_Position )
	{
		return static_cast< ValueType* >( memset( &*( Begin() + a_Position ), 0, sizeof( ValueType ) ) );
	}

	inline void Zero( size_t a_Position, size_t a_Length )
	{
		for ( Iterator Beg( Begin() + a_Position ); a_Length > 0; --a_Length, ++Beg )
		{
			memset( &*Beg, 0, sizeof( ValueType ) );
		}
	}
	...
	auto& operator=( const Enumerable< ValueType >&& a_Enumerable )
	{
		m_Operator( m_Begin, 0, 0, OperatorType::FDelete );
		m_Operator( m_End,   0, 0, OperatorType::FDelete );
		m_Size     = a_Enumerable.m_Size;
		m_Operator = a_Enumerable.m_Operator;
		m_Begin    = m_Operator( a_Enumerable.m_Begin, 0, 0, OperatorType::FClone );
		m_End      = m_Operator( a_Enumerable.m_End,   0, 0, OperatorType::FClone );
	}
...
public:
	...
	ReadOnlyEnumerable( ReadOnlyEnumerable& a_ReadOnlyEnumerable )
		: m_Begin( a_Enumerable.m_Operator( a_Enumerable.m_Begin, 0, 0, OperatorType::FClone ) )
		, m_End( a_Enumerable.m_Operator( a_Enumerable.m_End, 0, 0, OperatorType::FClone ) )
		, m_Size( a_Enumerable.m_Size )
		, m_Operator( a_Enumerable.m_Operator )
	...
	ReadOnlyEnumerable( CIter a_Begin, CIter a_End )
		: m_Begin( new Iter( a_Begin ) )
		, m_End( new Iter( a_End ) )
		, m_Size( distance( a_Begin, a_End ) )
		, m_Operator( Operator< Iter > )
	...
	template < typename CIter >
	ReadOnlyEnumerable( CIter a_Begin, CIter a_End, size_t a_Size )
		: m_Begin( new Iter( a_Begin ) )
		, m_End( new Iter( a_End ) )
		, m_Size( a_Size )
		, m_Operator( Operator< Iter > )
	...
	template < typename Container, typename CIter = ContainerTraits::GetCIter< Container >, typename = ContainerTraits::EnableIfContainer< Container, ValueType > >
	ReadOnlyEnumerable( const Container& a_Container )
		: m_Begin( new Iter( ContainerTraits::Begin( a_Container ) ) )
		, m_End( new Iter( ContainerTraits::End( a_Container ) ) )
		, m_Size( ContainerTraits::Size( a_Container ) )
		, m_Operator( Operator< Iter > )
	...
	// NEEDS WORK
	inline auto Combine( const ReadOnlyEnumerable< ValueType >& a_Enumerable ) const
	{
	...
	inline auto Difference( const ReadOnlyEnumerable< ValueType >& a_Enumerable ) const
	{
		set< const ValueType*, LessPtr< ValueType > > SetA;
		set< const ValueType*, LessPtr< ValueType > > SetB;
		vector< CReference< ValueType > > Result;
	...
	inline bool Equals( const ReadOnlyEnumerable< ValueType >& a_Enumerable )
	{
		if ( m_Size != a_Enumerable.m_Size )
		{
			return false;
	...
	inline auto Intersection( const ReadOnlyEnumerable< ValueType >& a_Enumerable ) const
	{
		set< const ValueType*, LessPtr< ValueType > > SetA;
		set< const ValueType*, LessPtr< ValueType > > SetB;
		vector< CReference< ValueType > > Result;
	...
	inline void Swap( ReadOnlyEnumerable< ValueType >& a_Enumerable )
	{
		swap( m_Begin, a_Enumerable.m_Begin );
		swap( m_End, a_Enumerable.m_End );
		swap( m_Size, a_Enumerable.m_Size );
	...
	inline auto Symmertry( const ReadOnlyEnumerable< ValueType >& a_Enumerable ) const
	{
		set< const ValueType*, LessPtr< ValueType > > SetA;
		set< const ValueType*, LessPtr< ValueType > > SetB;
		vector< CReference< ValueType > > Result;
