...
public:
	...
	using DelegateType = Delegate< Return, Args... >;
...
public:
	...
	/// <summary>
	/// Get the invoker at a specific index. Will fail if index out of range.
	/// </summary>
	inline InvokerType& operator[] ( size_t a_Index )
	{
		return *GetIterator( a_Index );
	}
	...
	/// for added invoker. Will add immediately.
	/// </summary>
	...
	/// <summary>
	/// Add an invoker onto the end of the invocation list. Will delay add until invocation
	/// has finalised.
	/// </summary>
	...
	/// <summary>
	/// Add all invokers in a delegate onto the end of the invocation list. Will delay add until invocation 
	/// has finalised.
	/// </summary>
	void operator+=( const DelegateType& a_Delegate )
	{
		if ( m_IsInvoking )
		{
			m_ToAdd.insert( m_Invokers.end(), a_Delegate.begin(), a_Delegate.end() );
		}
		else
		{
			m_Invokers.insert( m_Invokers.end(), a_Delegate.begin(), a_Delegate.end() );
	...
	/// <summary>
	/// Add a lambda function onto the end of the invocation list. Will delay add until invocation 
	/// has finalised.
	/// </summary>
	template < typename Lambda >
	...
	/// <summary>
	/// Add a static function onto the end of the invocation list. Will delay add until invocation
	/// has finalised.
	/// </summary>
	void operator+=( StaticFunction a_StaticFunction )
	{
		if ( m_IsInvoking )
		{
			m_ToAdd.emplace_back( a_StaticFunction );
		}
		else
		{
			m_Invokers.emplace_back( a_StaticFunction );
...
private:
	...
	void CleanUp()
	{
		...
		for ( auto Iterator = m_ToAdd.begin(); Iterator != m_ToAdd.end(); ++Iterator )
		{
			m_Invokers.push_back( *Iterator );
		}
